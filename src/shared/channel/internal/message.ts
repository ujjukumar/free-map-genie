...
export const MESSAGE_ID_FULL_LENGTH =
    MESSAGE_ID_PREFIX.length + MESSAGE_ID_LENGTH;
...
export type FillInternalMessage = {
    [K in keyof InternalMessage]?: () =>
        | Required<InternalMessage[K]>
        | Promise<Required<InternalMessage[K]>>;
};

export function prettyMessage(message: InternalMessage): string {
    return `message ${message.messageId} [${formatEndpointName(
        message.sender
    )} -> ${formatEndpointName(message.target)}]{ type: ${message.type} }`;
}
...
export function isValidMessageId(messageId: string): messageId is MessageID {
    return (
        ...
        messageId.length === MESSAGE_ID_FULL_LENGTH
    );
}

export function hopMessage(
    message: InternalMessage,
...
export function logMessage(message: InternalMessage) {
    ...
    logger.raw(
        `[${formatEndpointName(message.sender)} -> ${formatEndpointName(
            message.target
        )}]`
    );
